...
class EAttentionBaseline(nn.Module):
    ...
    def forward(self, x):
        ...
        second_offset = torch.zeros_like(offset)
        second_offset[..., 0] = torch.clamp(offset[..., 0] + torch.randint(-1, 2, offset[..., 0].shape, device=device), -1,
                                            1)
        second_offset[..., 1] = torch.clamp(offset[..., 1] + torch.randint(-1, 2, offset[..., 1].shape, device=device), -1,
                                            1)


        adjacent_mask = (second_offset[..., 0] != 0) | (second_offset[..., 1] != 0)
        second_offset = torch.where(adjacent_mask.unsqueeze(-1), second_offset, offset)
        ...
        if self.offset_range_factor >= 0:
            ...
            pos_second = second_offset + pos
        else:
            pos = (offset + reference).clamp(-1., +1.)
            pos_second = (second_offset + pos).clamp(-1., +1.)
        ...
        x_sampled_second = F.grid_sample(
            input=x.reshape(B * self.n_groups, self.n_group_channels, H, W),
            grid=pos_second[..., (1, 0)],  # y, x -> x, y
            mode='bilinear', align_corners=True)  # B * g, Cg, Hg, Wg

        x_sampled = x_sampled.reshape(B, C, 1, n_sample)
        x_sampled_second = x_sampled_second.reshape(B, C, 1, n_sample)
        x_sampled_combined = x_sampled + x_sampled_second

        q = q.reshape(B * self.n_heads, self.n_head_channels, H * W)
        k = self.proj_k(x_sampled_combined).reshape(B * self.n_heads, self.n_head_channels, n_sample)
        v = self.proj_v(x_sampled_combined).reshape(B * self.n_heads, self.n_head_channels, n_sample)
